...
async def test_first_evaluation_does_not_notify(db, monkeypatch):
    ...
    await measurements_repo.insert_measurement(db, MeasurementCreate(
        ...
        latency_ms=10.0, packet_loss=0.0, jitter_ms=None, success=True,
    ))
...
async def test_transition_to_offline_notifies(db, monkeypatch):
    ...
    await measurements_repo.insert_measurement(db, MeasurementCreate(
        target_id=target.id, timestamp=now.isoformat(), latency_ms=10.0, packet_loss=0.0, jitter_ms=None, success=True,
    ))
    await connectivity_service.evaluate(db, AppSettings())  # establishes baseline: online

    await measurements_repo.insert_measurement(db, MeasurementCreate(
        target_id=target.id, timestamp=now.isoformat(), latency_ms=None, packet_loss=100.0, jitter_ms=None, success=False,
    ))
    online = await connectivity_service.evaluate(db, AppSettings())
...
async def test_transition_back_online_notifies(db, monkeypatch):
    ...
    await measurements_repo.insert_measurement(db, MeasurementCreate(
        target_id=target.id, timestamp=now.isoformat(), latency_ms=8.0, packet_loss=0.0, jitter_ms=None, success=True,
    ))
    online = await connectivity_service.evaluate(db, AppSettings())
